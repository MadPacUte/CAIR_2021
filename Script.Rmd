...
```{r, out.width= "100%", out.height= "100%"}
# First we will need to get the data into a better format to see how gender and hs gpa is distributed
...
# If we look at only male/female within the data we can see the distribution 
...
# Now we will look at how to summarize gender and ethnicity based on hs gpa
...
gpa_ethnicity_gender_summarized %>%
  ...
  scale_y_continuous(labels = comma) +
  theme(
    axis.text.x = element_text(angle = 70, vjust = 0.5, hjust = 1, size = 5),
...
gpa_ethnicity_gender_year <-
  data_clean %>%
  extract(period, into = c("year", "semester"), "(.{4})(.{1})") %>%
  group_by(age, hs_gpa, comp_sex, ethnicity, year) %>%
  summarize(total = n()) %>%
  ungroup() %>%
  mutate(age = as.character(age)) %>%
  tidyr::pivot_wider(names_from = comp_sex, values_from = total, values_fill = 0) %>%
...
gpa_ethnicity_gender_year_summarized <-
  gpa_ethnicity_gender_year %>%
  select(hs_gpa:female) %>%
  group_by(ethnicity, hs_gpa, year) %>%
  summarize(
    male = sum(male),
    female = sum(female)
  ) %>%
...
gpa_ethnicity_gender_year_summarized %>%
  pivot_longer(cols = male:female, names_to = "gender", values_to = "count") %>%
  ggplot(aes(year, count, group = hs_gpa, color = hs_gpa)) +
  geom_line() +
  facet_wrap(~ ethnicity + gender, scales = "free_y", labeller = labeller(group = label_wrap_gen(width = 50))) +
  scale_y_continuous(labels = comma) +
  theme(
    axis.text.x = element_text(angle = 70, vjust = 0.5, hjust = 1, size = 5),
    axis.text.y = element_text(size = 5)
  ...
  labs(
    ...
    y = "",
    color = "HS GPA",
    title = "Time Series of HS GPA by Ethnicity and Gender"
  )
```
