...
# We picked out age, gpa, and ethnicity

data_summarized <-
  data_clean %>% # take the dataset
  group_by(age, gpa, ethnicity) %>% # group the data by the categories
...
```{r, out.width= "100%", out.height= "100%"}
# Now we will look at how to summarize ethnicity based on gpa
...
gpa_ethnicity_summarized <-
  ...
  select(gpa:total) %>% # select the columns that we would like to look at
  group_by(ethnicity, gpa) %>% # group the data by ethnicity and gpa
  summarize(
    total = sum(total) # add up all of the counts by the grouping
  ) %>%
  ungroup() %>% # remove the group by
...
gpa_ethnicity_summarized %>% # take the summarized gpa and ethnicity dataset
  ggplot(aes(gpa, total, fill = ethnicity)) + # initially plot the x and y coordinates and fill by ethnicity
  ...
  labs( # update the labels within the graph
    x = "",
    y = "",
    title = "Distribution of gpa by Ethnicity",
...
gpa_ethnicity_year <-
  ...
  group_by(age, gpa, ethnicity, year) %>% # group the data by age, gpa, ethnicity, and year
